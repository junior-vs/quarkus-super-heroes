...
public class FightService {
	...
	public FightService(HeroClient heroClient, 
			VillainClient villainClient, 
			@RestClient NarrationClient narrationClient,
			LocationClient locationClient,
			@Channel("fights") MutinyEmitter<io.quarkus.sample.superheroes.fight.schema.Fight> emitter,
			FightConfig fightConfig, FightMapper fightMapper) {
	...
	/**
	 * Adds a pre-configured delay to a response. Can be used to showcase/demo how
	 * to add delays as well as how to use fault tolerance.
	 * 
	...
	Uni<Fighters> addDelay(Uni<Fighters> fighters) {
		...
		return (delayMillis > 0) ? fighters.onItem().delayIt().by(Duration.ofMillis(delayMillis))
				.invoke(() -> Log.debugf("Adding delay of %d millis to request", delayMillis)) : fighters;
	}
	...
	Uni<Fight> persistFight(@SpanAttribute("arg.fight") Fight fight) {
		...
		return Fight.persist(fight)
				...
				.invoke(f -> this.emitter.sendMessageAndForget(
						Message.of(f, Metadata.of(TracingMetadata.withCurrent(Context.current())))))
				.replaceWith(fight);
	}
